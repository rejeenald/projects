...
class SCReports:
    ...
    def save_report_to_db(self, report:pd.DataFrame, table_name:str) -> None:
        ...
        db = DBClient()
        logging.debug(f"date start: {self.date_setter.start_date}")
        start_date = self.__format_date__(date_value=str(self.date_setter.start_date))
        logging.debug(f"date end: {self.date_setter.end_date}")
        end_date = self.__format_date__(date_value=str(self.date_setter.end_date))
        db.save_to_table(table_name=table_name, report=report, start_date=start_date, end_date=end_date)
        logging.info(f"Report saved to database: table name: {table_name}")

    def __format_date__(self, date_value) -> str:
        logging.info(f"Formatting date to %m-%d-%Y: {date_value}")
        date_value = dt.datetime.strptime(date_value, "%Y-%m-%d")
